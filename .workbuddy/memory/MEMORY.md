...
- **柔和配色（2026-08-16 用户定，共 4 主题）**：纯白扎眼/纯黑压抑 → 新增「雾沙」mist（**米黄调**浅色 #EDE7D3/#F5F0DE/#3F3B30/#5F7D72，低对比护眼）与「青墨」jade（**豆瓣绿调**深色 #18241D/#212E25/#C9D6C2/#93C878）。**「跟随系统」已按用户要求移除**（_resolve_theme 对未知/旧 system 值落回 mist）；默认主题 = mist。BADGE_STYLES 同步四套（mist 徽章底色偏暖黄、jade 偏豆绿）。主题卡 4 张一行（s(138)），迷你预览色按主题查 THEMES 表绘制
