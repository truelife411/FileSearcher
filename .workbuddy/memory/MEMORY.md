...
- **柔和配色（2026-08-16 定稿，共 4 主题）**：纯白扎眼/纯黑压抑 → 新增「雾沙」mist（米黄浅色 #EDE7D3/#F5F0DE/#3F3B30/#5F7D72）与「天青」jade（**青瓷浅色** #E6EDE9/#F1F5F1/#33413C/#5C8A7F，用户从 10+ 个候选方案中拍板选定，替换原青墨深色）。**「跟随系统」已按用户要求移除**（_resolve_theme 对未知/旧 system 值落回 mist）；默认主题 = mist。BADGE_STYLES 同步四套（天青徽章为浅色青瓷系）。主题卡 4 张一行（s(138)），迷你预览色按主题查 THEMES 表绘制。设计图脚本 theme_preview2.py 可复用（候选色板 → 模拟窗口+色板 PNG）
