...
- **USN 实时同步（2026-08-16 分支 feature/usn-sync 定稿规格）**：NTFS 变更日志增量同步替代手动/定时重建。**实测结论：读 USN 不需要管理员**（fsutil 降权验证 readJournal 成功），仅**创建 journal 需管理员**（D/E 盘已激活，C/F 原有；32MB 上限受 fsutil 版本限制不可调，截断自动全量重建兜底）。实现：`usn_sync.py` 独立模块（ctypes 卷枚举/query/read/create、USN_RECORD_V2 解析、UsnState 按卷序列号持久化 last_usn、UsnSyncEngine 事件分类→IndexEngine API）+ IndexEngine 扩展（dir_refs 映射表「父引用号→目录路径」、_scan_tree 递归新增、_remove_tree 递归删、_rescan_one_dir 一层 diff、rescan_dirs 批量、count_all）。**事件语义**：删除→remove_paths；重命名同引用号配对→rename_path（级联免费）；新建/修改→dirty 父目录→批量重扫。**翻译失败自愈**：深层新目录事件翻译失败→父目录重扫时 _scan_tree 递归覆盖。**轮询**：启动 3s 补差 + 定时循环（root.after，**间隔可配置 `usn_sync_interval_sec` 10~600 秒步进器默认 60**，_usn_interval_ms 钳制），后台线程，_index_running 时顺延；断档/换卷 → stats.reset → _do_index_silent 全量重建；同步只写库不刷新视图（快照零打扰）。**设置页内容区为可滚动容器**（content_canvas+scroll_frame+滚动条；滚轮 bind_all 接管非 Treeview 控件、打开前 unbind_all 防累积、切页 yview_moveto(0) 归位、宽度跟随 canvas）。**状态栏常驻同步状态（用户定格式）**：`索引已同步（N 项变更）. M 个文件 . 更新于 YYYY-MM-DD HH:MM:SS`（N=本轮变更数，M=count_all 索引条目总数，时间=完成时刻）；启动时显示「索引同步中…」，断档重建显示「索引重建中…」，失败显示「索引同步失败」；**搜索/复制/删除等所有活动信息经 _set_status 拼接在同步状态后**（直连 status_var.set 已全部改走 _set_status，仅 _set_status 内部两行保留直连）。设置页：删「启动自动重建/托盘自动重建/间隔步进器」，新增「自动同步索引」开关（auto_sync_index 默认 True）+「USN 状态」卡（各盘状态+激活按钮，非管理员提示提权）。旧设置项引用全清（托盘定时器代码删除）。**坑位**：① BY_HANDLE_FILE_INFORMATION 的 FILETIME 必须 2×DWORD（c_int64 在 x64 触发 8 字节对齐 padding → nFileIndex 错位）② USN 记录刷盘有秒级延迟（轮询间隔 60s 无感，测试要等 15-20s）③ 卷路径必须 `\\.\C:`（带尾斜杠 161 无效）④ 轮询语义：用**上次保存的 last_usn** 作起点（query 的 next_usn 是末尾，从它读永远 0 条）⑤ %TEMP% 在 IGNORE_PATH_CONTAINS 默认排除（TaskInfoEx 类风暴天然隔离）⑥ 首次接触卷只初始化游标不触发重建。**拖拽功能已删除**（tkinterdnd2 依赖+spec collect_all 清理）。验证脚本 _sync_test.py（6 用例单元）、_e2e_sync.py（真实事件全链路，临时库 monkeypatch INDEX_DB）
...
- **呼吸式表格 FileTable**：行分隔线、选中行 selected 底+左侧 6px accent 指示条、类型徽章（BADGE_STYLES[theme][kind]=(fg,bg)）、表头加粗+排序 accent+自绘小三角、**表头列名全部居中（2026-08-16 用户要求：文件名/路径/类型/大小/修改时间；header_align 全 center，排序三角 center 分支 = tx+lw/2+10）**、列宽拖动（_fit_cols 按序压缩/拉宽、_drag_pref 刚性列、_fitted_width 1:1 手感、layout.json 跨显示器换算）。**路径列用 _path_middle_ellipsis 中间省略**（保留首尾目录）
